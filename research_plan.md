...
>
> **C1 (현상):** 인간의 행동 보류는 passive omission이 아니라 체계적이고 선택적인 현상이며, learned-search agent와 질적으로 구별된다. \
...
### 1-1. 기본 현상: Human vs. Thinker action distribution

*리뷰어 관점: IL thinker와 비교해야 한다. Pretrained thinker와의 차이는 policy 자체의 차이일 수 있으므로, imitation 이후에도 gap이 남는지가 핵심이다.*

#### 확보된 결과 (sub001 ses04, SpaceInvaders)
- Human NOOP: **38.7%**, Thinker NOOP: **22.4%**, gap: **+16.0pp** (12개 파일, 73,161 real steps)
- NOOP bout 총 **3,413개**, 평균 bout 길이 **7.94 real steps**
- Human–thinker action distribution JSD 평균: **0.049** (non-trivial divergence)
- IL thinker 포함 3-way 비교에서 imitation 후에도 NOOP gap 유지 확인

#### 의미 및 가설 연결
Human과 thinker 간 NOOP gap은 imitation learning 후에도 유지된다. 이는 단순한 policy 차이가 아닌 **human-specific timing strategy**의 존재를 시사하며, 본 논문의 출발 관찰(C1)을 구성한다. JSD 0.049는 action distribution 전반에 걸친 non-trivial divergence를 quantify한다.

#### 한계 및 후속 필요
- 현재 sub001 단일 피험자 수준 → **N=6 전체 확장 필요** ⚠️
- Imitation thinker 내부 tree statistics와 NOOP gap의 관계 정량화 (Section 2로 연결)
...
#### 의미 및 가설 연결
1-3에서 텍스트로만 서술한 "heavy-tail → planned delay" 주장을 **피험자별 시각적 증거**로 뒷받침한다. 지수분포에서의 이탈은 매 step의 NOOP가 독립적으로 결정되지 않음을 의미하며, C1(현상의 체계성)과 C2(structured postponement)를 연결하는 bridge 역할을 한다.
...
#### 핵심 질문
- NOOP onset 직전에 uncertainty가 상승하는가?
- NOOP 기간 동안 search conflict가 유지되는가?
- Commit 직전에 action confidence가 올라가는가? (resolution signal)

#### 필요한 세부 연구
- Real step 기준으로 onset/commit aligned temporal profile 추출 **[V → Figure 2B/2C에 구현됨]**
- **Uncertainty의 "rise-and-fall" 패턴 검증**: onset에서 peak, commit에서 drop → directional t-test 또는 Wilcoxon **[NEW]**
- **Commit 직전 action confidence (top-2 Q gap, policy margin)가 onset 직후보다 높은지 통계 검증** **[NEW]**
...
### 3-3. Causal ordering: Uncertainty → NOOP, or NOOP → uncertainty change?

*리뷰어 관점: Cross-sectional correlation은 인과성을 보장하지 않는다. 가능하면 temporal precedence를 보여야 한다.*

> **2-3과의 관계**: 2-3은 bout *내부*에서 uncertainty가 어떻게 감소하는지를 기술했다. 3-3은 bout *onset 이전* window에서 uncertainty가 선행하는지를 검증한다. 두 분석은 서로 다른 시간 창에서 같은 인과 방향(uncertainty → withholding)을 보완적으로 지지한다.
>
...
- **Granger causality test** 또는 **lagged regression**: t-1 uncertainty → t NOOP probability (temporal precedence 확인) **[NEW]**
- **Pre-bout vs. control baseline 비교**: bout 직전 uncertainty가 matched control보다 높은가? (already partially done but needs formal test) **[V → 강화 필요]**
- **Bout onset entropy → bout length**: AR-residual의 보완 — uncertainty가 bout을 시작시킬 뿐 아니라 지속시키는가 **[NEW]**
...
| 1-1 | Action distribution: human vs. thinker (bar chart) | **Fig 1-1B** (`fig_1-1_B_action_distribution.png`) | ✅ Done |
...
| 2-1 | Residual distribution gap: human vs. IL thinker NOOP gap + JSD (imitation thinker 필요) | **Fig 1-1D** | ✅ Done (1-1에서 이동) |
