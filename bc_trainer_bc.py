...
def compute_enhanced_bc_losses(outputs, batch_data, model_net, flags, device):
    ...
    if vpn_policy is not None and target_actions is not None:
        ...
        vpn_policy_weight = 1.0  # Hardcoded weight
...
def run_bc_training_step(model_net, actor_net, batch_data, flags, device):
    ...
    # Build Python tree manager to mirror Thinker's Cython tree logic
    tree_manager = TreeManager(batch_size=batch_size, num_actions=num_actions, flags=flags, device=device)

    root_encoded = []
    for idx in range(batch_size):
        encoded = {"real_states": obs[idx]}
        if initial_xs is not None:
            encoded["xs"] = initial_xs[idx]
        if initial_hs is not None:
            encoded["hs"] = initial_hs[idx]
        root_encoded.append(encoded)

    if initial_model_out.policy is not None:
        init_policy = initial_model_out.policy[0] if len(initial_model_out.policy.shape) > 1 else initial_model_out.policy
        if init_policy.dim() == 3:
            init_policy = init_policy.squeeze(1)
    else:
        init_policy = torch.full((batch_size, num_actions), 1.0 / num_actions, device=device)

    if initial_model_out.vs is not None:
        init_values = initial_model_out.vs[0] if len(initial_model_out.vs.shape) > 1 else initial_model_out.vs
    ...
    init_rewards = (
        initial_model_out.rs[0]
        if hasattr(initial_model_out, "rs") and initial_model_out.rs is not None
        else None
    )

    tree_manager.expand_root(init_rewards, init_values, init_policy, root_encoded)
    tree_reps = tree_manager.compute_tree_reps()

    # Track rollout state
    current_obs = obs.float()
    current_xs = initial_xs
    ...
    for step in range(rec_t - 1):
        ...
        # TreeManager keeps track of "current time" and rollout depth internally
        ...
        # Update model state snapshot for the next iteration
        if isinstance(model_out.state, dict):
            model_state = {k: (v.clone() if hasattr(v, "clone") else v) for k, v in model_out.state.items()}
        else:
            model_state = model_out.state

        # 2. Update Python tree manager with latest model statistics
        encoded_payload = []
        for idx in range(batch_size):
            ...
            if hs is not None:
                payload["hs"] = hs[idx]
            encoded_payload.append(payload)

        logits = model_out.policy[0] if model_out.policy is not None else torch.zeros(batch_size, num_actions, device=device)
        if logits.dim() == 3:
        ...
        values_step = model_out.vs[0] if model_out.vs is not None else torch.zeros(batch_size, device=device)
    ...
    # Final tree update for the "real" step
    ...
    # Debug: show the real-step action chosen by the ActorNet
    real_action_probs = None
    if hasattr(final_actor_out, 'action_prob') and final_actor_out.action_prob is not None:
        real_action_probs = final_actor_out.action_prob
        while real_action_probs.dim() > 2:
            real_action_probs = real_action_probs[0]
    elif hasattr(final_actor_out, 'pri_param') and final_actor_out.pri_param is not None:
        logits = final_actor_out.pri_param
        while logits.dim() > 2:
            logits = logits[0]
        real_action_probs = torch.softmax(logits, dim=-1)

    if real_action_probs is not None:
        real_step_actions = torch.argmax(real_action_probs, dim=-1)
        unique_real_actions = torch.unique(real_step_actions).cpu().numpy()
        first_batches = [real_action_probs[i].detach().cpu().numpy() for i in range(min(3, real_action_probs.shape[0]))]
        print(f"[DEBUG] Real step actions predicted: {unique_real_actions}")
        print(f"[DEBUG] Real step action probs (first 3 batches): {first_batches}")
...
def run_bc_training(flags, model_net, actor_net, logger=None):
    ...
    proj_name = getattr(flags, 'proj_name', 'bc_checkpoints')
    bc_log_dir = os.path.join(project_root, 'logs', 'thinker', proj_name)
    os.makedirs(bc_log_dir, exist_ok=True)
    print(f"BC checkpoints will be saved to: {bc_log_dir}")

    # Save config file to project-specific BC directory
