#!/usr/bin/env python3
"""
02_structure_analysis_cognitiveuncertainty.py  –  Section 2 with Cognitive Uncertainty

Same analysis as 02_structure_analysis.py, but uncertainty is redefined:
  Instead of actor policy entropy, cognitive uncertainty is defined as
  the action diversity (entropy) of human actions taken at the 100 most
  similar real states, where similarity is measured in PCA space derived
  from im_vectors (global-avg-pooled to 128-dim).
  To avoid temporal leakage, only states from OTHER files are used as
  neighbors.

Sub-sections: same as 02_structure_analysis.py (2-1 through 2-5).
Primary metric: uncertainty_cognitive  (replaces entropy_actor)
Secondary metric: entropy_actor  (kept for comparison)

Outputs (all in --out-dir): same file names as 02_structure_analysis.py.
"""
...
METRIC_LABELS = {
    "uncertainty_cognitive": "Cognitive Uncertainty",
    "entropy_actor": "Actor Entropy",
    "entropy_actor_prev_s2": "Prev-s2 actor entropy",
    ...
    "rollout_return_prev_s2": "Prev-s2 rollout return",
}
...
def _set_data_driven_xlim(
    ...
    *,
    ...
    if span <= 1e-9 * scale:
        ...
        half_span = max(min_span / 2.0, abs(center) * pad_fraction)
        left, right = center - half_span, center + half_span
    else:
        pad = max(span * pad_fraction, 1e-6 * scale)
        left, right = vmin - pad, vmax + pad
...
# ─── Cognitive uncertainty computation ────────────────────────────────────────

def pool_im_vector(raw) -> np.ndarray:
    """Convert im_vector (1,128,6,6) to global-avg-pooled (128,) representation."""
    arr = np.asarray(raw, dtype=float)
    if arr.ndim == 4:
        arr = arr[0]  # (128, 6, 6)
    elif arr.ndim == 5:
        arr = arr[0, 0]  # handle extra batch dim if present
    return arr.reshape(128, -1).mean(axis=1)  # (128,)


def build_cognitive_uncertainty_index(
    paths: List[Path],
    n_pca: int = 50,
    n_neighbors: int = 100,
    extra_query_factor: int = 5,
) -> Dict[Tuple[str, int], float]:
    """
    Compute cognitive uncertainty for every real step across all files.

    Cognitive uncertainty = Shannon entropy of the human action distribution
    among the n_neighbors most similar real states in PCA-reduced im_vector
    space, restricted to states from OTHER files.

    Returns a dict keyed by (file_str, global_step_idx) -> float.
    """
    print("Building cognitive uncertainty index...")
    print(f"  Phase 1: collecting real-step im_vectors from {len(paths)} files...")

    records: List[Tuple[str, int, np.ndarray, int]] = []
    for p in paths:
        ...
        t = min(len(status), len(human_action), len(im_vectors))
        file_str = str(p)
        for idx in real_idx:
            if idx >= t:
                continue
            vec = pool_im_vector(im_vectors[int(idx)])
            records.append((file_str, int(idx), vec, int(human_action[int(idx)])))
        print(f"    {p.name}: {len(real_idx)} real steps")

    if len(records) == 0:
        print("  No real steps found.")
        return {}

    all_files = np.array([r[0] for r in records])
    all_vecs = np.stack([r[2] for r in records])   # (N, 128)
    all_actions = np.array([r[3] for r in records])
    n_total = len(records)
    n_unique_files = len(set(all_files.tolist()))

    if n_unique_files < 2:
        print("  Warning: only 1 file — cross-file uncertainty undefined. Returning NaN.")
        return {(r[0], r[1]): np.nan for r in records}

    print(f"  Phase 2: PCA on {n_total} real steps (128d → {n_pca}d)...")
    n_components = min(n_pca, all_vecs.shape[1], n_total - 1)
    pca = PCA(n_components=n_components, random_state=42)
    X_pca = pca.fit_transform(all_vecs)
    explained = float(pca.explained_variance_ratio_.sum())
    print(f"    Variance explained by {n_components} PCs: {explained:.3f}")

    print(f"  Phase 3: KNN (k={min(n_total - 1, n_neighbors * extra_query_factor)}) ...")
    k_query = min(n_total - 1, n_neighbors * extra_query_factor)
    nn = NearestNeighbors(n_neighbors=k_query + 1, algorithm="auto", n_jobs=-1)
    nn.fit(X_pca)
    _, indices = nn.kneighbors(X_pca)   # (N, k_query+1)

    n_actions = max(int(all_actions.max()) + 1, 6)

    print("  Phase 4: computing cross-file action diversity...")
    result: Dict[Tuple[str, int], float] = {}
    n_insufficient = 0
    for i, (file_id, global_idx, _, _) in enumerate(records):
        nbr_idx = indices[i][1:]  # exclude self (index 0)
        cross_mask = all_files[nbr_idx] != file_id
        cross_nbrs = nbr_idx[cross_mask][:n_neighbors]

        if len(cross_nbrs) < 5:
            result[(file_id, global_idx)] = np.nan
            n_insufficient += 1
            continue

        nbr_actions = all_actions[cross_nbrs]
        counts = np.bincount(nbr_actions, minlength=n_actions)
        probs = counts / float(counts.sum())
        diversity = float(-np.sum(probs * np.log(probs + EPS)))
        result[(file_id, global_idx)] = diversity

    print(f"  Done. {n_total} real steps indexed, {n_insufficient} with insufficient cross-file neighbors.")
    return result
...
def build_real_step_table(
    ...
    file_path: Path,
    cognitive_uncertainty_map: Dict[Tuple[str, int], float],
    ...
    root_ns = np.asarray(tree.get("root_ns", np.zeros_like(root_policy)), dtype=float).reshape(len(status), -1)
    ...
    t = min(
        ...
        len(root_policy), len(cur_policy), len(root_ns), len(rollout_return),
    ...
    root_policy = root_policy[:t]; cur_policy = cur_policy[:t]; root_ns = root_ns[:t]
    ...
    for i, idx_global in enumerate(real_idx):
        ...
        var_rollout_return = (
            float(np.var(pre_reset_returns)) if len(pre_reset_returns) >= 2 else np.nan
        )
        ...
        cog_unc = cognitive_uncertainty_map.get((file_str, int(idx_global)), np.nan)
        ...
        rows.append({
            ...
            "uncertainty_cognitive": float(cog_unc) if np.isfinite(cog_unc) else np.nan,
            ...
            "var_rollout_return": float(var_rollout_return) if np.isfinite(var_rollout_return) else np.nan,
            "rollout_return_prev_s2": float(rollout_return[prev_s2]),
            "max_rollout_return_prev_s2": float(max_rollout_return[prev_s2]),
...
def extract_noop_bouts(df_real: pd.DataFrame) -> pd.DataFrame:
    ...
    for file_id, g in df_real.groupby("file", sort=False):
        ...
        for b_ix, seg in enumerate(split_contiguous(noop_pos)):
            ...
            def _get(col: str, pos: int) -> float:
                return float(g.loc[pos, col]) if col in g.columns else np.nan
            ...
            rows.append({
                ...
                # Cognitive uncertainty (primary). At NOOP onset itself the action is
                # mechanically NOOP, so onset uncertainty uses the immediately prior state.
                "uncertainty_at_pre": _get("uncertainty_cognitive", pre),
                "uncertainty_at_onset": _get("uncertainty_cognitive", pre),
                "uncertainty_at_noop_onset_raw": _get("uncertainty_cognitive", s),
                "uncertainty_at_commit": _get("uncertainty_cognitive", commit),
                # Actor entropy (for comparison)
                "entropy_at_pre": _get("entropy_actor", pre),
                "entropy_at_onset": _get("entropy_actor", s),
                "entropy_at_commit": _get("entropy_actor", commit),
                "margin_at_pre": _get("margin_actor", pre),
                "margin_at_onset": _get("margin_actor", s),
                "margin_at_commit": _get("margin_actor", commit),
                "q_gap_at_pre": _get("q_gap", pre),
                "q_gap_at_onset": _get("q_gap", s),
                "q_gap_at_commit": _get("q_gap", commit),
                "rollout_spread_at_pre": _get("rollout_spread", pre),
                "rollout_spread_at_onset": _get("rollout_spread", s),
                "rollout_spread_at_commit": _get("rollout_spread", commit),
            })
...
def build_event_tables(
    ...
    metrics = list(metric_cols or [
        "uncertainty_cognitive",
        "entropy_actor", "entropy_actor_prev_s2",
        "margin_actor", "margin_actor_prev_s2",
        "q_gap_prev_s2", "rollout_spread_prev_s2", "rollout_spread",
    ])
    ...
    for _, b in bouts.iterrows():
        ...
        for m in metrics:
            ...
            raw_on_val = float(g.loc[onset_pos, m])
            on_val = pre_val if m == "uncertainty_cognitive" else raw_on_val
            com_val = float(g.loc[commit_pos, m])
            rec[f"{m}_pre"] = pre_val
            rec[f"{m}_onset"] = on_val
            if m == "uncertainty_cognitive":
                rec[f"{m}_noop_onset_raw"] = raw_on_val
...
# ─── Section 2-1: Onset uncertainty vs. bout length ──────────────────────────
...
def plot_fig_2_1c(bouts: pd.DataFrame, out_path: Path) -> None:
    """Fig 2-1C: Onset cognitive uncertainty vs. bout length scatter + regression."""
    bouts = annotate_cross2_bouts(bouts)

    fig, ax = plt.subplots(figsize=(5.8, 4.5))
    x_sc = bouts["uncertainty_at_onset"].to_numpy(dtype=float)
    ...
    if mask.sum() > 5 and np.unique(x_sc[mask]).size > 1:
        slope, intercept, r, p, _ = scipy_stats.linregress(x_sc[mask], y_sc[mask])
        xx = np.linspace(np.nanmin(x_sc[mask]), np.nanmax(x_sc[mask]), 100)
        ax.plot(xx, slope * xx + intercept, color="#d62728", lw=2,
                label=f"r = {r:.3f}, p = {p:.3g}")
    _set_data_driven_xlim(ax, x_sc)
    ax.set_xlabel("Pre-onset cognitive uncertainty")
    ax.set_ylabel("log(1 + bout length)")
    ax.set_title("Fig 2-1C: Pre-onset cognitive uncertainty vs. bout length")
...
def plot_fig_2_2(
    ...
    out_path: Path,
    metric: str = "uncertainty_cognitive",
    variant_label: Optional[str] = None,
) -> None:
    """Fig 2-2: 4-panel temporal profile figure."""
...
def plot_fig_2_2_1_short_long(
    ...
    """Fig 2-2.1: Short/Long cognitive uncertainty using pre-onset as onset."""
    bouts_c2 = annotate_cross2_bouts(bouts)
    prepost_c2 = prepost.merge(
        bouts_c2[["event_id", "bout_class"]], on="event_id", how="left"
    )
    metric_label = _metric_label(metric)
    onset_col = f"{metric}_onset"
    pre_col = f"{metric}_pre"
    commit_col = f"{metric}_commit"
    delta_commit_onset_col = f"{metric}_delta_commit_onset"

    if metric == "uncertainty_cognitive" and pre_col in prepost_c2.columns:
        prepost_c2[onset_col] = prepost_c2[pre_col]
        if commit_col in prepost_c2.columns:
            prepost_c2[delta_commit_onset_col] = prepost_c2[commit_col] - prepost_c2[pre_col]

    fig, axes = plt.subplots(1, 3, figsize=(15.0, 4.5))
    class_specs = [("Short", "#1f77b4"), ("Long", "#ff7f0e")]
    rng = np.random.default_rng(42)

    def _plot_class_values(ax, col: str, title: str, ylabel: str) -> None:
        if len(prepost_c2) == 0 or col not in prepost_c2.columns:
            ax.set_axis_off()
            return
        for x_pos, (cls, color) in enumerate(class_specs):
            vals = prepost_c2.loc[prepost_c2["bout_class"] == cls, col].to_numpy(dtype=float)
            vals = vals[np.isfinite(vals)]
            if len(vals) == 0:
                continue
            jitter = rng.normal(0.0, 0.035, size=len(vals))
            ax.scatter(np.full(len(vals), x_pos) + jitter, vals, s=9, alpha=0.18, color=color)
            mean = float(np.mean(vals))
            sem = float(np.std(vals, ddof=1) / np.sqrt(len(vals))) if len(vals) > 1 else 0.0
            ax.errorbar([x_pos], [mean], yerr=[sem], marker="o", ms=7, capsize=4,
                        color=color, lw=2, label=f"{cls} (n={len(vals)})")
        ax.set_xticks([0, 1], ["Short", "Long"])
    ...
    _plot_class_values(
        axes[0], onset_col,
        "Fig 2-2.1A: Pre-onset uncertainty by class",
        f"Pre-onset {metric_label}",
    )
    _plot_class_values(
        axes[1], commit_col,
        "Fig 2-2.1B: Commit uncertainty by class",
        f"Commit {metric_label}",
    )

    ax = axes[2]
    if (
        len(prepost_c2) > 0
        and onset_col in prepost_c2.columns
        and delta_commit_onset_col in prepost_c2.columns
    ):
        for cls, color in class_specs:
            sub = prepost_c2[prepost_c2["bout_class"] == cls]
            x = sub[onset_col].to_numpy(dtype=float)
            y = -sub[delta_commit_onset_col].to_numpy(dtype=float)
            mask = np.isfinite(x) & np.isfinite(y)
            ax.scatter(x[mask], y[mask], s=9, alpha=0.22, color=color,
                       label=f"{cls} points (n={mask.sum()})")
            if mask.sum() > 5 and np.nanstd(x[mask]) > 0:
                slope, intercept, r, p, _ = scipy_stats.linregress(x[mask], y[mask])
                xx = np.linspace(np.nanmin(x[mask]), np.nanmax(x[mask]), 100)
                ax.plot(xx, slope * xx + intercept, color=color, lw=2,
                        label=f"{cls}: r={r:.3f}, p={p:.3g}")
        _set_data_driven_xlim(ax, prepost_c2[onset_col])
        ax.axhline(0, color="black", lw=1, ls="--")
        ax.set_xlabel(f"Pre-onset {metric_label.lower()}")
        ax.set_ylabel(f"Confidence gain\n(-delta {metric_label.lower()})")
        ax.set_title("Fig 2-2.1C: Pre-onset uncertainty vs. confidence gain")
    ...
    for metric_name in metrics:
        for phase in phases:
            ...
            for cls in ["Short", "Long"]:
                ...
                vals = vals[np.isfinite(vals)]
                summary_rows.append({
                    "metric": metric_name, "phase": phase, "bout_class": cls,
                    "n": int(len(vals)),
            ...
            test_rows.append({
                "metric": metric_name, "phase": phase,
                "n_short": int(len(short)), "n_long": int(len(long)),
                "mean_short": float(np.mean(short)) if len(short) else np.nan,
                "mean_long": float(np.mean(long)) if len(long) else np.nan,
                "long_minus_short": float(np.mean(long) - np.mean(short)) if len(short) and len(long) else np.nan,
...
def compute_bout_internal_trajectory(
    ...
    metric_cols = ["uncertainty_cognitive", "entropy_actor", "margin_actor", "q_gap", "rollout_spread"]
    rows = []
    for _, b in bouts_c2.iterrows():
        ...
        row = {
            "event_id": b["event_id"], "file": b["file"],
            "subject": b["subject"], "game": b["game"],
            "bout_length": bout_len, "cross2": int(b["cross2"]),
            "is_long": int(b["is_long"]), "bout_class": b["bout_class"],
        }
        for metric_name in metric_cols:
            if metric_name not in g.columns:
                continue
...
def compute_bout_commit_dynamics(df_real: pd.DataFrame, bouts: pd.DataFrame) -> pd.DataFrame:
    ...
    metric_cols = [
        "uncertainty_cognitive", "entropy_actor", "margin_actor",
        "q_gap", "rollout_spread", "entropy_actor_prev_s2",
        "margin_actor_prev_s2", "q_gap_prev_s2",
    ]
    rows = []
    for _, b in bouts_c2.iterrows():
        g = grouped[b["file"]]
        ...
        mid = int(round((s + e) / 2))
        stages = {"pre": pre, "onset": s, "mid": mid, "final_noop": e, "commit": commit}
        row = {
            "event_id": b["event_id"], "file": b["file"],
            "subject": b["subject"], "session": b["session"],
            "game": b["game"], "bout_length": int(b["length_real_steps"]),
            "cross2": int(b["cross2"]), "is_long": int(b["is_long"]),
...
def compute_episode_stats(df_real: pd.DataFrame) -> pd.DataFrame:
    ...
    for file_id, g in df_real.groupby("file", sort=False):
        ...
        cog_r, cog_slope = _safe_corr_slope(g["uncertainty_cognitive"], g["is_human_noop"])
        ...
        rows.append({
            "file": file_id,
            "subject": meta.subject, "session": meta.session,
            "block": meta.block, "game": meta.game,
            "chunk": _safe_int(meta.chunk),
            "n_steps": len(g),
            "noop_ratio": float(g["is_human_noop"].mean()),
            "mean_cognitive_uncertainty": float(g["uncertainty_cognitive"].mean()),
            ...
            "cognitive_uncertainty_noop_r": cog_r,
            "cognitive_uncertainty_noop_slope": cog_slope,
...
def compute_half_session_stats(df_real: pd.DataFrame) -> pd.DataFrame:
    ...
    for file_id, g in df_real.groupby("file", sort=False):
        ...
        for half, sub in [("Early", g.iloc[:mid]), ("Late", g.iloc[mid:])]:
            cog_r, cog_slope = _safe_corr_slope(sub["uncertainty_cognitive"], sub["is_human_noop"])
            ...
            rows.append({
                "file": file_id, "subject": meta.subject,
                "session": meta.session, "game": meta.game, "half": half,
                "n_steps": int(len(sub)),
...
def compute_noop_commit_policy_trajectory(
    ...
    rows = []
    metrics_to_track = [c for c in ["tree_width", "var_rollout_return"]
                        if c in next(iter(grouped.values())).columns]
    for _, b in bouts_c2.iterrows():
        g = grouped[b["file"]]
        pre, commit = int(b["pre_pos"]), int(b["commit_pos"])
        if pre < 0 or commit >= len(g) or commit < pre:
            continue
        seg = g.loc[pre:commit, metrics_to_track]
        if len(seg) < 2:
            continue
        ...
        row = {
            ...
            "cross2": int(b["cross2"]), "is_long": int(b["is_long"]),
            "bout_class": b["bout_class"],
            "n_real_steps_pre_to_commit": int(len(seg)),
        }
...
def plot_fig_2_4_noop_commit_policy_trajectory(
    ...
    """Fig 2-4: search-value trajectory from pre-NOOP onset state to commit."""
    metric_specs = []
    if any(c.startswith("tree_width_pos") for c in traj.columns):
        metric_specs.append(("tree_width", "Tree width",
                              "Fig 2-4A: Tree width trajectory"))
    if any(c.startswith("var_rollout_return_pos") for c in traj.columns):
        metric_specs.append(("var_rollout_return", "Var. rollout return",
                              "Fig 2-4B: Rollout-return variance trajectory"))

    n_panels = max(len(metric_specs), 1)
    fig, axes = plt.subplots(1, n_panels, figsize=(5.9 * n_panels, 4.6), sharex=True)
    if n_panels == 1:
        axes = [axes]

    for ax, (metric_name, label, title) in zip(axes, metric_specs):
        pos_cols = sorted(
            [c for c in traj.columns if c.startswith(f"{metric_name}_pos")],
            key=lambda c: int(c.rsplit("pos", 1)[1]),
        )
    ...
    for _, b in bouts_c2.iterrows():
        ...
        for pos in range(s, e + 1):
            ...
            row = {
                "event_id": b["event_id"], "file": b["file"],
                "subject": b["subject"], "game": b["game"],
                "real_pos": int(pos), "global_idx": int(g.loc[pos, "global_idx"]),
                "bout_length": bout_len, "cross2": int(b["cross2"]),
                "is_long": int(b["is_long"]), "bout_class": b["bout_class"],
...
def compute_commit_hazard_models(
    ...
    models = [
        ...
        ("Cognitive uncertainty", ["log_elapsed_noop_steps", "uncertainty_cognitive"]),
        ...
        ("Cog. uncertainty + prev-s2 tree", [
            "log_elapsed_noop_steps", "uncertainty_cognitive",
            "q_gap_prev_s2", "rollout_spread_prev_s2",
        ]),
        ("Actor + prev-s2 tree", [
            "log_elapsed_noop_steps", "entropy_actor",
            "q_gap_prev_s2", "rollout_spread_prev_s2",
        ]),
        ("Imag search summary", [
    ...
    for model_name, predictors in models:
        ...
        avail_cols = [c for c in cols if c in hazard.columns]
        if len(avail_cols) < len(cols):
            continue
        data = hazard[avail_cols].replace([np.inf, -np.inf], np.nan).dropna().reset_index(drop=True)
...
def plot_fig_2_5_commit_hazard(
    ...
    ax.barh(np.arange(len(ordered_loss)), ordered_loss["log_loss_improvement_vs_elapsed"],
    ...
    for ax, model_name, title in [
        (axes[2], "Cog. uncertainty + prev-s2 tree", "Fig 2-5C: Cog. uncertainty + final search state"),
...
def compute_section2_summary(bouts: pd.DataFrame) -> pd.DataFrame:
    ...
    for (subject, game), g in bouts_c2.groupby(["subject", "game"], sort=True):
        ...
        def _mean_col(col: str, mask: np.ndarray) -> float:
            return float(g.loc[mask, col].mean()) if mask.sum() > 0 and col in g.columns else np.nan
        ...
        rows.append({
            ...
            "mean_cogunc_onset_short": _mean_col("uncertainty_at_onset", short_mask),
            "mean_cogunc_onset_long": _mean_col("uncertainty_at_onset", long_mask),
            "mean_cogunc_commit_short": _mean_col("uncertainty_at_commit", short_mask),
            "mean_cogunc_commit_long": _mean_col("uncertainty_at_commit", long_mask),
            "mean_entropy_onset_short": _mean_col("entropy_at_onset", short_mask),
            "mean_entropy_onset_long": _mean_col("entropy_at_onset", long_mask),
            "mean_entropy_commit_short": _mean_col("entropy_at_commit", short_mask),
            "mean_entropy_commit_long": _mean_col("entropy_at_commit", long_mask),
        })
    return pd.DataFrame(rows)
...
def run_section2_cognitive(
    ...
    # Phase 1: build cognitive uncertainty index across all files
    cognitive_uncertainty_map = build_cognitive_uncertainty_index(
        paths, n_pca=n_pca, n_neighbors=n_neighbors,
    )

    # Phase 2: build real-step tables with cognitive uncertainty
    print(f"Building real-step tables for {len(paths)} files...")
    all_real = []
    for p in paths:
        df = build_real_step_table(
            load_npy_dict(p), p, cognitive_uncertainty_map, k_rewards=(5,),
        )
        if len(df) > 0:
            all_real.append(df)
    if not all_real:
    ...
    prepost, temporal = build_event_tables(
        ...
        metric_cols=[
            ...
            "entropy_actor_prev_s2", "margin_actor_prev_s2",
            "q_gap_prev_s2", "rollout_spread", "rollout_spread_prev_s2",
            "tree_width", "var_rollout_return",
        ],
    )
    ...
    noop_commit_policy_traj.to_csv(out_dir / "noop_commit_search_trajectory.csv", index=False)
    ...
    plot_fig_2_2_1_short_long(bouts, temporal, prepost,
    ...
    plot_fig_2_4_noop_commit_policy_trajectory(noop_commit_policy_traj,
                                               out_dir / "fig_2_4_precommit_search_trajectory.png")

    print(f"\nSection 2 (cognitive uncertainty) analysis complete → {out_dir}")
    print(summary.to_string(index=False))


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Section 2: NOOP Bout Structure Analysis with Cognitive Uncertainty"
    )
    p.add_argument("--input-dir", type=Path, default=Path("test/sub001/ses-04"),
                   help="Directory containing .npy files")
    p.add_argument("--out-dir", type=Path,
                   default=Path(__file__).parent / "outputs" / "02_structure_analysis_cognitiveuncertainty",
    ...
    p.add_argument("--cross2-summary", type=Path,
                   ...
                   help="Optional subject×game Cross2 summary from Section 1")
    p.add_argument("--game-id", type=int, default=None,
                   help="Optional game filter, e.g. 1 for game1 files")
    p.add_argument("--window-pre", type=int, default=6,
                   help="Temporal window before onset/commit")
    p.add_argument("--window-post", type=int, default=6,
                   help="Temporal window after onset/commit")
    p.add_argument("--n-pca", type=int, default=50,
                   help="Number of PCA components for im_vector representation")
    p.add_argument("--n-neighbors", type=int, default=100,
                   help="Number of nearest cross-file neighbors for diversity metric")
    return p.parse_args()


def main() -> None:
    args = parse_args()
    run_section2_cognitive(
