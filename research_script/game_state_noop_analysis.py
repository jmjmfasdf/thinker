...
SpaceInvaders
  ...
  28  threat level (enemy group lowest Y; 35 = wave start, ~117 = very close)
...
OUT_DIR   = Path(__file__).parent / "outputs" / "game_state_noop"
...
SI_RAM = {
    ...
    "enemies_y":    24,   # enemy group lowest Y: 16=wave start, ~146=very close (threat)
    "num_lives":    73,   # lives remaining (0–3)
}

# Enemies Y baseline (wave start value)
ENEMIES_Y_BASELINE = 16
