...
def record_entry(entries, status, real_img, im_img, tree_reps, real_vec, im_vec):
    entry = {
        "status": int(status),
        "real_img": real_img,
        "im_img": im_img,
...
def run_planning_step(obs_stack, action_index, model_net, actor_net, flags, device, num_actions, dim_actions, copy_ch):
    ...
    record_entry(entries, 0, last_frame.copy(), im_root.copy(), decoded_root, real_vec, im_vec)
    ...
    last_action = torch.tensor([action_index], dtype=torch.long, device=device)
    ...
    for step in range(flags.rec_t - 1):
        ...
        env_out.done = torch.zeros(1, 1, dtype=torch.bool, device=device)
        env_out.real_done = torch.zeros(1, 1, dtype=torch.bool, device=device)
        ...
        record_entry(entries, status_value, last_frame.copy(), current_im.copy(), decoded, real_vec_step, im_vec_step)
...
def main():
    ...
    def emit_stack():
        nonlocal current_frames, current_actions, processed_steps
        if not current_frames:
            return

        frames_to_stack = list(current_frames)
        stack_input = build_frame_stack(frames_to_stack, frame_stack, blank_frame)
        action_index = current_actions[-1] if current_actions else 0

        entries = run_planning_step(stack_input, action_index, model_net, actor_net, flags, device, num_actions, dim_actions, copy_ch)
