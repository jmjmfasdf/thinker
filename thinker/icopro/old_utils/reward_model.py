...
# device = 'cuda'  # NOTE: fix this, use hydra's cfg.device now
...
class RewardModel:
    def __init__(self, ds, da, is_atari=False,
        ...
        self.capacity = int(capacity)  # query buffer capacity

        if is_atari:
            self.buffer_seg1_img = np.empty((self.capacity, size_segment, *self.ds), dtype=np.float32)
            self.buffer_seg1_a = np.empty((self.capacity, size_segment, 1), dtype=np.float32)
            self.buffer_seg2_img = np.empty((self.capacity, size_segment, *self.ds), dtype=np.float32)
            self.buffer_seg2_a = np.empty((self.capacity, size_segment, 1), dtype=np.float32)
        else:
        ...
        self.raw_actions = []
        self.img_inputs = []
        self.mb_size = mb_size  # reward batch size
        self.origin_mb_size = mb_size  # reward batch size may change according to the current training step
    ...
    def add_data(self, obs, act, rew, done):
        ...
        # NOTE: each elements in self.inputs or self.targets corresponding to a whole trajectory
        init_data = len(self.inputs) == 0
        if init_data:
            self.inputs.append(flat_input)
            self.targets.append(flat_target)
        elif done:
            # If done and not init_data, it's impossible that self.inputs[-1]==[], 
            # since it's impossible to have two consecutive 'done' in two steps.
            # Therefore here could use concatenate directly without worrying concat([[], flat_input])
            ...
            if len(self.inputs[-1]) == 0:  # the last element is [] since len([])==0, which means last trajectory has done
    ...
    def r_hat_member(self, x, member=-1):  # NOTE: have grad to back propagate
        # the network parameterizes r hat in eqn 1 from the paper
        return self.ensemble[member](torch.from_numpy(x).float().to(self.device))

    def r_hat(self, x):  # NOTE: no grad
    ...
    def get_queries(self, mb_size=20):
        ...
        # len_traj: length og each trajectory # TODO: does every trajectory has the same len_traj?
        len_traj, max_len = len(self.inputs[0]), len(self.inputs)
        img_t_1, img_t_2 = None, None
        
        if len(self.inputs[-1]) < len_traj:  # TODO: why???
            max_len = max_len - 1
    ...
    def get_label(self, sa_t_1, sa_t_2, r_t_1, r_t_2):
        sum_r_t_1 = np.sum(r_t_1, axis=1)  # TODO: along the time dimension, right?
