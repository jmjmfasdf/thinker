...
def _stack_onehots(onehots, num_actions):
    if len(onehots) == 0:
        return None
    valid = next((oh for oh in onehots if oh is not None), None)
    if valid is None:
        return None
...
def _seed_video_stats(video_stats, env_out, env, render, copy_n, tree_reps, current_step_times, save_encoder_vectors, device, actor_policy_logit_init, reset_policy_logit_init):
    ...
    video_stats["tree_reps"].append({k: v.detach().cpu().numpy() for k, v in tree_reps.items()})
    ...
    return root_real_states, root_xs, last_reward_to_log
...
def visualize(
    ...
    if data_path:
        ...
        target_size = (images.shape[1], images.shape[2])
        num_actions = actions.shape[-1] if actions.ndim == 2 else int(actions.max()) + 1
        base_env = BehaviorDatasetVectorEnv(
            images=images,
            actions=actions,
            rewards=rewards,
            is_first=is_first,
    ...
    while len(returns) < max_eps_n:
        ...
        im_dict["pri_logits"].append(actor_out.pri_param[:, 0].detach().cpu())
        im_dict["reset_logits"].append(actor_out.reset_logits[:, 0].detach().cpu())
        im_dict["pri"].append(actor_out.pri[:, 0].detach().cpu())
        im_dict["cur_reset"].append(actor_out.reset[:, 0].detach().cpu())
        
        tree_reps_ = env.decode_tree_reps(env_out.tree_reps)
        model_policy.append(tree_reps_["cur_policy"].detach().cpu())
       ...
       # ):
        ...
        if len(video_stats["real_imgs"]) < max_video_frames:
            ...
            video_stats["tree_reps"].append(
                {k: v.detach().cpu().numpy() for k, v in tree_reps.items()}
            )
        ...
        if torch.any(env_out.real_done):
            ...
            if savevideo:
                saved = _save_video_stats(
                    ...
                    outdir,
                    env.num_actions,
                    f"video_stat_{video_stat_idx:03d}.npy",
                )
                if saved:
                    video_stat_idx += 1
                video_stats = _init_video_stats(save_encoder_vectors)
...
if __name__ == "__main__":
    ...
    if int(flags.max_steps) >= 0:
        flags.max_frames = flags.max_steps

    visualize(
        savedir=flags.savedir,
        xpid=flags.xpid,
        outdir=flags.outdir,
        data_path=flags.data if flags.data else None,
        plot=False,
        saveimg=True,
        savevideo=True,
        seed=flags.seed,
