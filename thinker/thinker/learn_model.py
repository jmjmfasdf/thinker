...
class SModelLearner:
    ...
    def _init_bc_components(self):
        """Prepare human-behaviour dataset access for future IRL use."""
        self.bc_loader = None
        ...
        try:
            ...
            self._logger.warning(f"Invalid icopro_subjects '{subjects_raw}'; skipping behaviour dataset setup.")
            return
        if not subjects:
            self._logger.warning("No valid icopro_subjects provided; skipping behaviour dataset setup.")
            return
        ...
        if len(self.bc_loader.data_files) == 0:
            self._logger.warning("IcoPro model data loader found no files; skipping behaviour dataset setup.")
            self.bc_loader = None
            return
        self.bc_data_ready = True
        self._logger.info(
            "IcoPro model data loader initialised with %d files (subjects=%s, game_id=%d). "
            "Supervised losses are disabled, but batches remain available for IRL.",
            len(self.bc_loader.data_files),
            subjects,
            game_id,
        )
    ...
    def _init_actor_bc_components(self):
        ...
        self.bc_actor_rollouts_ready = False

        if self.bc_loader is None:
            return
        if self.actor_param_buffer is None:
            self._logger.warning("Actor param buffer unavailable; behaviour rollouts disabled.")
            return
        actor_param = None
        try:
        ...
        if actor_param is None:
            ...
            for _ in range(50):
                ...
                if actor_param is not None:
                    break
        if actor_param is None:
            self._logger.warning("Unable to fetch actor init params; behaviour rollouts disabled.")
            return
        ...
        try:
            ...
            self._logger.warning(
                f"Failed to move behaviour actor net to device '{self.icopro_device}': {exc}. Falling back to CPU."
            )
            self.icopro_device = torch.device("cpu")
            setattr(self.flags, "icopro_device", "cpu")
            self.bc_actor_net.to(self.icopro_device)
        self.bc_actor_net.train(True)
        self.bc_actor_policy_adapter = ThinkerPolicyAdapter(
            self.bc_actor_net, self.model_net, self.flags, self.icopro_device
        )
        self.bc_actor_rollouts_ready = True
        self._logger.info("Behaviour rollout adapter initialised (loss computation disabled).")
    ...
    def collect_model_behavior_batch(self):
        """
        Return the next batch of stacked human observations without computing supervised losses.

        The batch is sourced from FrameStackedBehavioralDataLoader and left on the learner
        device so downstream IRL code can decide how to process it.
        """
        return self._sample_bc_batch()
    ...
    def collect_actor_behavior_rollout(self, requires_grad: bool = False):
        """
        Sample human demonstrations and run the Thinker policy adapter without computing supervised losses.

        Args:
            requires_grad: When True, keep autograd graph for downstream objectives. Defaults to False.

        Returns:
            Dictionary containing the raw batch, policy outputs, Q-values, and rollout metadata.
        """
        if self.bc_loader is None or self.bc_actor_policy_adapter is None:
            return None
        batch = self._sample_actor_bc_batch()
        if batch is None:
            return None
        if self.actor_param_buffer is not None and self.bc_actor_net is not None:
            try:
                weights = ray.get(self.actor_param_buffer.get_data.remote("actor_net"))
            except Exception:
                weights = None
        ...
        if requires_grad:
            policy = self.bc_actor_policy_adapter.forward(**forward_kwargs)
        else:
            with torch.no_grad():
                policy = self.bc_actor_policy_adapter.forward(**forward_kwargs)
        tree_q = self.bc_actor_policy_adapter.last_tree_q
        if tree_q is not None and self.bc_actor_tree_coef != 0.0:
            q_values = policy.logits + self.bc_actor_tree_coef * tree_q
        else:
        ...
        return {
            ...
            "q_values": q_values,
            "tree_q": tree_q,
            "rollout_history": self.bc_actor_policy_adapter.last_rollout_history,
            "imagined_actions": self.bc_actor_policy_adapter.last_imagined_actions,
        }
